...
{
    return [[self getUsingLKDBHelper] search:self where:where orderBy:orderBy offset:offset count:count callback:block];
}
...
+(void)searchWithSQL:(NSString*)sql callback:(void (^)(NSMutableArray* array))block {
    return [[self getUsingLKDBHelper] searchWithRawSQL:sql toClass:self withBlock:block];
}
