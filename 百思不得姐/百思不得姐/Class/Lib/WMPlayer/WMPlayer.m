...
- (UIButton *)forwardBtn
{
    if (!_forwardBtn) {
        ...
        [forwardBtn setImage:[UIImage imageNamed:WMPlayerSrcName(@"forward")] ?: [UIImage imageNamed:WMPlayerFrameworkSrcName(@"forward")] forState:UIControlStateNormal];
        forwardBtn.backgroundColor = [UIColor colorWithWhite:0.2 alpha:0.7];
        forwardBtn.layer.cornerRadius = 5.0;
        CGFloat width = self.bounds.size.width;
        CGFloat height = self.bounds.size.height;
        forwardBtn.frame = CGRectMake((width - 80) * 0.5, (height - 30) * 0.5, 80, 30);
        forwardBtn.titleLabel.font = [UIFont systemFontOfSize:13.];
        forwardBtn.imageEdgeInsets = UIEdgeInsetsMake(0, -5, 0, 0);
...
//    UIActivityIndicatorViewStyleWhite 的尺寸是（22，22）
    ...
    self.progressSlider.bounds = CGRectMake(0, 0, 0, 0);
